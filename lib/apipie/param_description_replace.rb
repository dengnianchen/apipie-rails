module Apipie
  ...
  class ParamDescription
    ...
    attr_reader :method_description, :name, :desc, :allow_nil, :constraint, :options, :metadata, :show, :as
    attr_accessor :parent, :required

    def self.from_dsl_data( method_description, param_dsl )
      Apipie::ParamDescription.new( method_description, param_dsl )
    end

    def initialize( method_description, param_dsl )

      @method_description = method_description
      @name = concern_subst( param_dsl[:name] )

      @options = param_dsl[:options]
      @options[:desc] = @desc = param_dsl[:desc]
      ...
      @show = (@options.has_key? :show) ? @options[:show] : true
      ...
      @constraint = Type::TypeConstraint.create( param_dsl[:type], param_dsl[:block] )
    ...
    def validate( value )
      raise ParamError.new( "the value cannot be null" ) if !@allow_nil && value.nil?
      @constraint.validate value
    end

    def process_value( value )
      return nil if value.nil?
      return @constraint.process_value( value ) if @constraint.respond_to?( :process_value )
      return value
    end
